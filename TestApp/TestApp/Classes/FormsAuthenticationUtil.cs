...
namespace TestApp
{
    public class FormsAuthenticationUtil
    {
        ...
        /// <param name="cookiePath">Path for which the authentication ticket is valid</param>
        private static void RedirectFromLoginPageMain(string userName, string commaSeperatedRoles, bool createPersistentCookie, string cookiePath)
        {
            SetAuthCookieMain(userName, commaSeperatedRoles, createPersistentCookie, cookiePath);
            //HttpContext.Current.Response.Redirect(FormsAuthentication.GetRedirectUrl(userName, createPersistentCookie), true);
        ...
        {
            RedirectFromLoginPageMain(userName, commaSeperatedRoles, createPersistentCookie, cookiePath);
        }
        ...
        private static FormsAuthenticationTicket CreateAuthenticationTicket(string userName, string commaSeperatedRoles, bool createPersistentCookie, string strCookiePath)
        {
            ...
            userName,               //Username for whome the ticket is issued
        ...
        /// <param name="createPersistentCookie">True or false whether to create persistant cookie</param>
        /// <param name="cookiePath">Path for which the authentication ticket is valid</param>
        private static void SetAuthCookieMain(string userName, string commaSeperatedRoles, bool createPersistentCookie, string cookiePath)
        {
            ...
            else
            {
                ...
                // class, the time-out can be set through the Expiration attribute. This value 
                // will override the timeout attribute value specified in configuration files.
